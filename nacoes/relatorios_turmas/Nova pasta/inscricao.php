...
$sql = "select a.*,b.nome,c.codigo as cod_mat,c.nota,c.frequencia,c.situacao,concat(d.descricao,' ',c.observacao) as disc,e.descricao as disc_curso from turmas a 
		...
		left join cadastro_cursos e on a.codigo_curso=e.codigo 
				where c.codigo_aluno='$cod' order by a.data_inicio desc,a.codigo";
...
echo "<td><input type='image' src='../img/enviar.gif' title='Imprimir comprovante de Matrícula' onclick=\"return imprimir_comprovante('$dados->cod_mat');\">";
...
echo "<td><input type='image' src='../img/enviar.gif' title='Imprimir declaração de Matrícula' onclick=\"return imprimir_declaracao('$dados->cod_mat');\">";
