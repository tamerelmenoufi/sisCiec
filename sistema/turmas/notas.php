...
if($_GET[At] == 1){
	mysql_query("update matricula set publicado = '1', integracao = '2', alteracao = NOW() where codigo_turma='".$_GET[cod]."'");
	//echo "update matricula set publicado = '1', integracao = '2', alteracao = NOW() where codigo_turma='".$_GET[cod]."'<br>";
}
if($_GET[At] == 2){
	mysql_query("update matricula set publicado = '0', integracao = '2', alteracao = NOW() where codigo_turma='".$_GET[cod]."'");
	//echo "update matricula set publicado = '0', integracao = '2', alteracao = NOW() where codigo_turma='".$_GET[cod]."'";
}
					   ...
					   if($_POST){
						  ...
						  for($i=0;$i<$n;$i++){
						    ...
						    $vt1 = $_POST[codigo1][$i];
								...
								$query = "update matricula set nota='".$_POST[nota][$vt]."', frequencia='".$_POST[frequencia][$vt]."', situacao='".$_POST[situacao][$vt]."', integracao = '2' where codigo_matricula = '".$vt."'";
								if(mysql_query($query)){ $soma++; }
								//echo $query."<br>";
                  ...
                  <tr>
                    <td>
                     ...
                     <?php
					   ...
					   $query = "select a.codigo, a.codigo_aluno, b.cci, b.nome, a.nota, a.frequencia, a.situacao /*, (select codigo from matricula where codigo_matricula = a.codigo) as vinculo*/ from matricula a
					   			left join cadastro_aluno b on a.codigo_aluno=b.codigo
					   			where a.codigo_turma='".$_GET['cod']."' and b.nome != '' and a.codigo_escola='$conf[codigo_curso]' group by a.codigo_aluno $com";
...
<?php						   
					   ...
					   $sql = "select codigo from matricula where codigo_matricula = '$dados->codigo'";
					 ...
					 ?>
					  <tr>
						...
						<input name="nota[<?=$dados->codigo?>]" value="<?=$dados->nota?>" type="text" size="5" onKeyUp="notas(this.value,'<?=$dados->codigo?>','f<?=$f?>')">
						<input type="hidden" name="codigo[]" value="<?=$dados->codigo?>">
						<input type="hidden" name="codigo1[]" value="<?=$dados->vinculo?>">
						</td>
						...
						<select name="situacao[<?=$dados->codigo?>]"  id="x<?=$dados->codigo?>">
                          <?php 
						       if($dados->situacao == 'MT'){ $mt = 'selected'; }else{ $mt = false; }
						       if($dados->situacao == 'AP'){ $ap = 'selected'; }else{ $ap = false; }
						       if($dados->situacao == 'RP'){ $rp = 'selected'; }else{ $rp = false; }
						       if($dados->situacao == 'FT'){ $ft = 'selected'; }else{ $ft = false; }
