...
echo "  <tr>";
echo "    <td colspan='5' class='titulos_modelos'>Declaração de disciplinas";
echo "         <td  align='right'>&nbsp;";
...
//AQUI COMEÇA O PROGRAMA 
	...
	$result = mysql_query($query);
		echo "<form action='declaracao_eliminacao.php' target='_blank' method='post' name='f'>\n";
   ...
   echo "</form>";
		...
		$lista = "select a.*, b.descricao from docs a left join cadastro_cursos b on a.codigo_curso=b.codigo where a.codigo_aluno='".$cod."' and a.tipo='declaracao_eliminacao' order by a.data desc";
		$lista = mysql_query($lista);
		while( $l = mysql_fetch_object($lista)){
			echo "<form action='declaracao_eliminacao.php' method='post' target='_blank'><input type='submit' name='acao' value='".$l->descricao." - emitido em  ".$l->data."'><input type='hidden' name='cod' value='$cod'><input type='hidden' name='curso' value='".$l->codigo_curso."'><input type='hidden' name='data' value='".$l->data."'></form>";	
		}
